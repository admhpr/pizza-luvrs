...
function encodePizza(pizza) {
  return { ...pizza, toppings: JSON.stringify(pizza.toppings) }
}
function decodePizza(pizza) {
  return { ...pizza, toppings: JSON.parse(pizza.toppings) }
}

function decodePizzas(pizzas) {
  return pizzas.map(decodePizza)
}
module.exports = {
  batchImport,
  create,
